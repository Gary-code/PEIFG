...
class peifgConfig(OPTConfig):
    model_type = "peifg"


class peifgOPTModel(OPTModel):
    config_class = peifgConfig

    def __init__(self, config: OPTConfig):
        super(peifgOPTModel, self).__init__(config)
    ...
    def initialize_vision_modules(
        ...
        image_processor = CLIPImageProcessor.from_pretrained('./pre-trained/vit-large-patch14')
        # 1024*1024
    ...
    def forward(
        ...
        return super(peifgOPTModel, self).forward(
...
class peifgOPTForCausalLM(OPTForCausalLM):
    config_class = peifgConfig
    # supports_gradient_checkpointing = True

    def __init__(self, config):
        super(OPTForCausalLM, self).__init__(config)
        self.model = peifgOPTModel(config)
...
AutoModelForCausalLM.register(peifgConfig, peifgOPTForCausalLM)
